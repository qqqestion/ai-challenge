...
**Описание:** Реализация агента с композицией MCP-инструментов для выполнения сложных задач через цепочки операций.

✨ **Что нового:**
- ✅ Композиция MCP-инструментов для выполнения сложных задач
- ✅ **Цепочки операций** - последовательное выполнение нескольких инструментов
- ✅ Планирование задач - автоматическое определение последовательности вызовов
- ✅ Обработка зависимостей между инструментами
- ✅ Передача результатов между шагами цепочки
- ✅ Обработка ошибок в цепочках операций

🎯 **Ключевая фича:** Агент может выполнять сложные многошаговые задачи, автоматически планируя последовательность вызовов MCP-инструментов и передавая результаты между шагами!
...
1. **Реализовать композицию MCP-инструментов** - создать систему для объединения нескольких инструментов в цепочки операций
2. **Планирование последовательности** - автоматически определять оптимальный порядок вызова инструментов для выполнения задачи
3. **Обработка зависимостей** - корректно обрабатывать зависимости между инструментами и передавать результаты между шагами
4. **Цепочки операций** - реализовать выполнение сложных многошаговых задач через последовательные вызовы инструментов
5. **Обработка ошибок в цепочках** - корректная обработка ошибок на любом этапе выполнения цепочки с возможностью отката или альтернативных путей
6. **Интеграция с LLM** - использовать LLM для планирования цепочек операций на основе естественного языка запроса
