...
⚠️ **ВАЖНО:** MCP интеграция **отключена по умолчанию** из-за известных проблем с lifecycle. Бот полностью работоспособен без MCP. Для включения MCP добавьте `MCP_ENABLED=true` в `.env` файл.
